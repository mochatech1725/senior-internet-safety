...
The Girl Scout's name is intentionally not included anywhere in the site for privacy.

## Cloudflare Pages
Upload this folder or its ZIP to Cloudflare Pages. The project name can be `seniorinternetsafety` if available, which would result in the default URL `seniorinternetsafety.pages.dev`.

There is no build step. `index.html` is at the root.
